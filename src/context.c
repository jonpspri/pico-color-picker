...
static void s_context_enable(context_t *c) {
  ...
  if(c->enable_ccb.callback) c->enable_ccb.callback(c, c->enable_ccb.data, (v32_t)0ul);
...
void context_builder_set_re(uint8_t re_offset, uint8_t button_offset,
    context_callback_f re_callback, void *re_data, context_callback_f button_callback, void *button_data, const char *label) {
...
void context_builder_set_button(uint8_t offset, context_callback_f callback, void *data, int16_t label) {
  context_t *c = (context_t *)pvTaskGetThreadLocalStoragePointer(NULL, ThLS_BLDR_CTX);
  ASSERT_IS_A(c, CONTEXT_T);
...
void context_builder_set_enable_callback(context_callback_f callback, void *data) {
  context_t *c = (context_t *)pvTaskGetThreadLocalStoragePointer(NULL, ThLS_BLDR_CTX);
  ASSERT_IS_A(c, CONTEXT_T);
...
void context_builder_set_display_callback(context_callback_f callback, void *data) {
  context_t *c = (context_t *)pvTaskGetThreadLocalStoragePointer(NULL, ThLS_BLDR_CTX);
  ASSERT_IS_A(c, CONTEXT_T);
...
void context_display_task(void *parm) {
  ...
  if(!screen_buffer) screen_buffer = bitmap_alloc(SCREEN_WIDTH, SCREEN_HEIGHT, b_ssd1306_init);
  ...
  for( ;; ) {
    ...
    if (!(c->display_ccb.callback)) continue;
    bitmap_clear(c->pane);
    ...
    bitmap_draw_string(screen_buffer, 0, RE_LABEL_Y_OFFSET, &TRIPLE_LINE_TEXT_FONT, c->re_labels[0]);
    bitmap_draw_string(screen_buffer,
        (RE_LABEL_TOTAL_WIDTH - TRIPLE_LINE_TEXT_FONT.Width*strnlen(c->re_labels[1],8))/2,
        RE_LABEL_Y_OFFSET, &TRIPLE_LINE_TEXT_FONT, c->re_labels[1]);
    bitmap_draw_string(screen_buffer,
        RE_LABEL_TOTAL_WIDTH - TRIPLE_LINE_TEXT_FONT.Width*strnlen(c->re_labels[2],8),
        RE_LABEL_Y_OFFSET, &TRIPLE_LINE_TEXT_FONT, c->re_labels[2]);

    /*
     * Draw the chevrons if they're there
     */
    bitmap_draw_char(screen_buffer, RE_LABEL_TOTAL_WIDTH, 0, &BUTTON_LABEL_FONT, c->button_chars[0]);
