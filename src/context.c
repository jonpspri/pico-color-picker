...
static void s_context_enable(context_t *c)
{
    ...
    if(c->enable_ccb.callback) c->enable_ccb.callback(c, c->enable_ccb.data,
            (v32_t) 0ul
            );
...
void context_display_task(void *parm)
{
    ...
    if(!screen_buffer) screen_buffer = bitmap_alloc(SCREEN_WIDTH, SCREEN_HEIGHT,
            b_ssd1306_init
            );
    ...
    for( ;;) {
        ...
        bitmap_draw_string(screen_buffer, 0, RE_LABEL_Y_OFFSET,
                &TRIPLE_LINE_TEXT_FONT, c->re_labels[0]
                );
        bitmap_draw_string(screen_buffer,
                ( RE_LABEL_TOTAL_WIDTH - TRIPLE_LINE_TEXT_FONT.Width*strnlen(
                        c->re_labels[1],8
                        ) )/2,
...
{
    if(!context_stack) context_stack = xQueueCreate( 10,
